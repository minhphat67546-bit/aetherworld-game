...
function registerPartyHandlers(io, socket) {
  socket.on('party_invite', (targetName) => {
    ...
    players.forEach((p, sid) => { if (p.name === targetName) targetSocketId = sid; });
  ...
  socket.on('party_accept', (fromId) => {
    ...
    party.members.forEach(mId => {
      io.to(mId).emit('party_update', { 
        partyId, members: party.members.map(id => players.get(id)?.name).filter(Boolean)
      });
    });
  });
  ...
  socket.on('party_leave', () => {
    ...
    if (party) {
      ...
      if (party.members.length === 0) {
        ...
        party.members.forEach(mId => {
          io.to(mId).emit('party_update', { 
            partyId: party.id, members: party.members.map(id => players.get(id)?.name).filter(Boolean)
          });
        });
      }
    }
